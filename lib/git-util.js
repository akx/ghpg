...
async function getConfig(key) {
  const value = await getOutput([getGitCommand(), 'config', key]);
  return value.trim();
}

async function getPreviousCommit(branch) {
  try {
    const rev = await getOutput([
      getGitCommand(),
      'rev-list',
      '--max-count=1',
      branch,
...
function push(remote, branch) {
  return spawnPromise(getGitCommand(), ['push', '-v', remote, branch], {
    stdio: 'inherit',
  });
}
